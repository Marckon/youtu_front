    ...
    Page: function (address, commentLength, pageIndex, eventCount, navigateCount, keyword) {
        ...
        $(".pageNumber").click(function () {
            if ($(this).text() != pageIndex) {
                window.location.href = ("searchALL.html?keyword=" + keyword + "&page=" + $(this).text())
            }
        });
        $(".list-page>.prev").click(function () {
            window.location.href = ("searchALL.html?keyword=" + keyword + "&page=" + (pageIndex - 1))

        });
        $(".list-page>.next").click(function () {
            window.location.href = ("searchALL.html?keyword=" + keyword + "&page=" + (pageIndex - 0 + 1))
        });
    }
...
$(function () {
    ...
    $.ajax({
        type: "GET",
        url: "youtu/front/user/likeMaster?describ=" + keyword + "&pageNum=1&pageSize=3",
        success: function (data) {
            ...
            $.ajax({
                type: "GET",
                url: "youtu/front/vedioSearch/vedioLike?describ=" + keyword + "&pageNum=" + pageNumber + "&pageSize=20",
                success: function (data) {
                    //获取搜索到的视频数量
                    var vedioLen = (data.data === null ? 0 : data.data[(data.data.length - 1)].totalSize);
                    //如果大于0先移除搜索失败的界面

                    if (vedioLen > 0) {
                        $(".search-wrapper").append('' +
                            '<div class="search-title"><span></span>相关视频</div>' +
                            '<div class="v-box clearfloat">' +
                            '<ul class="v-list ">' +
                            '</ul>' +
                            '</div>');
                        ...
                        if (vedioLen > 20) {
                            youtuVedio.Page("search-wrapper", vedioLen, pageNumber, 20, 5, keyword)
                        }
                    }
                    if (masterLen < 1 && vedioLen < 1) {
